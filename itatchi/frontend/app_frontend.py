...
# --- Dados Iniciais Mockados (apenas para fins de teste) ---
# IDs e nomes dos dados de teste inseridos no próprio SQL.
...
# As opções de categoria são apenas para referência no campo "Categoria" da tela
OPCOES_CATEGORIAS = ["Regulatórios", "Qualidade", "Pessoas", "Veículos", "Locais"] 


def formatar_data(data):
    """Formata objetos date para string YYYY-MM-DD, ou retorna None."""
    # Garante que None ou um tipo de dado inválido não causem erro
    if data and isinstance(data, (datetime, date)):
        return data.strftime('%Y-%m-%d')
    return None

st.set_page_config(layout="wide", page_title="Sistema Itatchi - Cadastro")
...
with st.form(key='cadastro_documento_form'):
    ...
    # Categoria é derivada para contexto
    ...
    # Datas e Metadados
    st.header("Validade e Metadados")
    col3, col4, col5 = st.columns(3)
    
    emissao = col3.date_input("Data de Emissão (opcional)", value=None)
    
    # Checkbox para 'Vigente até substituição' (sem_validade)
    sem_validade = col5.checkbox("Sem Validade Definida", value=False)
    
    # Validade: Campo Opcional (RF-1)
    # Se 'Sem Validade' está marcado, desabilita a entrada de data de validade
    validade = None
    if not sem_validade:
        validade = col4.date_input("Data de Validade (opcional)", value=None, min_value=date.today())
        # Garante que se a pessoa não escolher data, mas não marcar "sem validade", validade é None
        if validade is None:
            col4.warning("Recomendado informar a Validade para documentos regulatórios.")

    # Outros Metacampos (fins de teste)
    col6, col7 = st.columns(2)
    numero = col6.text_input("Número do Documento (Alvará/Apólice)", placeholder="000123/2025")
...
if submit_button:
    ...
    if not titulo or not responsavel:
        ...
        payload = {
            ...
            # Envia a validade se a caixa 'Sem Validade' NÃO estiver marcada
        ...
        try:
            # Envia para a rota POST /documentos
            ...
            if response.status_code == 201:
                ...
                # Trata erros do Flask (como formato de data inválido ou erro interno)
